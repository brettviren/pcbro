...
local spfilts = import "sp-filters.jsonnet";

function(anode, resp) g.pnode({
    type: 'OmnibusSigProc',
    name: anode.name + 'sigproc%d' % anode.data.ident,
    data: {
        // Many parameters omitted here.
        anode: wc.tn(anode),
        field_response: wc.tn(resp.field_resp),
        ...
        isWrapped: false,
        // use dupind=true option for RawSource
        process_planes: [1, 2, 0],
    },
}, nin=1, nout=1, uses=[anode, resp.field_resp, resp.elec_resp]+spfilts)
