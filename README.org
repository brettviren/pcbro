...
PCBro provides scripts to map these data to the real geometry.  This mapping is illustrated in the following diagram
...
#+begin_example
$ wirecell-pcbro convert-garfield \
  [...slice...options...] \
  garfield-pcb.tar pcbro-response-<name>.json.bz2
#+end_example

This should run with no errors about "No response for plane ...".

The ~slice options~ determine which slice or an average of both slices
are used for a given plane.  The provided script [[file:scripts/gen-response.sh][gen-response.sh]] will
iterate over a few combinations producing these ~<name>~ files with the
given slice assignments:

| ~<name>~         | U     | V     | W     |
| ~indslc-colave~  | 0     | 1     | <0,1> |
| ~indave-colave~  | <0,1> | <0,1> | <0,1> |
| indave-colscl0 | <0,1> | <0,1> | 0     |
| indave-colscl1 | <0,1> | <0,1> | 1     |

Where "<0,1>" means average over the two slices for induction (U and
V) or collection (W).

All these combinations may be produced, including generating some diagnostic plots with:

#+begin_center
  $ ./scripts/gen-response.sh
  $ ls cfg/pcbro-response-*
#+end_center
